...
function solveSudoku(sudoku) {
  ...
  function updateState(index, value, type) {
    ...
    var subGridRow = parseInt(row / subGridHeight),
      ...
      subGridIndex =
        subGridRow * subGridHeight * width + subGridColumn * subGridWidth;
  ...
  function hiddenSingle() {
    ...
    for (var index = 0; index < length; index++) {
      ...
      var subGridIndex =
        parseInt(row / subGridHeight) * subGridHeight * width +
        parseInt(column / subGridWidth) * subGridWidth;
      for (var i = 0; i < possibleValue[index].length; i++) {
        isUniqueNumber = true;
        UniqueFlag: for (var j = 0; j < subGridHeight; j++) {
  ...
  function tryNumber(operation) {
    ...
    for (var i = 0; i < possibleArray.length && !stopLoop; i++) {
      ...
      for (var j = 0; j < arr.length && !stopLoop; j++) {
        ...
        loopSolveSudoku();
        if (
          !possibleValue.some(function(arr) {
